...
const editFolderIndex = (pathIndex, dim) => {
  fs.readFile(pathIndex, 'utf-8', function (err, data) {
    ...
    const dw = splits[0]
    const dh = splits[1]
    let indexContext = data.replace(/<\/style>/g, '.main_container { width: ' + dw + 'px; height: ' + dh + 'px; }</style>')

    indexContext = indexContext.replace(/(?<=<meta name=\"ad.size\" )(.*?)(?=>)/, 'content="width=' + dw + ',height=' + dh + '"')
...
const generateFolder = async (destDir, dim) => {
  ...
  // console.log('generateFolder', fse)
...
async function startGenerate(options) {
  ...
  _dimensions.forEach(async (obj) => {
    await generateFolder(path.join(_getDir, options.campaignName + '_' + obj), obj)
  })
...
const date_created = moment.tz(new Date(), 'America/New_York').format('MMMM DD, YYYY h:mA z')
...
function generatePreview(options) {
  ...
  // copy gifs
  fse.copySync(path.join(_getDir, 'gif'), path.join(_distDir, 'gif'), { overwrite: true })
...
function generateMainIndex(options, INDEX_CURLY, folders, gifs) {
  ...
  fse.copy('./_template/cent_index.html', path.join(_distDir, 'index.html'), { overwrite: true }, function (err) {
    if (err) throw err
    ...
    fs.readFile(pathIndex, 'utf-8', function (err, data) {
      ...
      folders.forEach((obj) => {
        ...
        str += '\n<option value="' + name + '" w="' + dw + '" h="' + dh + '" src="' + name + '/index.html">' + name + '</option>'
      })
      ...
      gifs.forEach((obj) => {
        ...
        str += '\n<option value="' + name + '" w="' + dw + '" h="' + dh + '" src="gif/' + obj + '">' + obj + '</option>'

        hasGif = true
      })

      let indexContext = curlyStr.replace(/(?<=\<select(.*)>)(.*)(?=\<\/select>)/s, str)
...
function editMainIndex(options, INDEX_CURLY, folders, gifs) {
  ...
  fs.readFile(pathIndex, 'utf-8', function (err, data) {
    ...
    let updated = moment.tz(new Date(), 'America/New_York').format('MMMM DD, YYYY h:m A z')
    let indexContext = data.replace(/(?<=<span class=\"updated\">)(.*?)(?=<\/span>)/g, updated)

    let htmlCntr = folders.length
    let gifsCntr = gifs.length

    indexContext = indexContext.replace(/(?<=class="text-end">)(.*?)(?=<\/span>)/, htmlCntr + ' html<br>' + gifsCntr + ' gif')
    ...
    indexContext = indexContext.replace(/(?<=\<select(.*)>)(.*)(?=\<\/select>)/s, str)
...
function removeDStore(options, INDEX_CURLY, folders, gifs) {
  ...
  folders.forEach((obj) => {
    ...
    fs.rm(path.join(_distDir, obj, '.DS_Store'), { recursive:true }, (err) => {
      ...
      console.log('DS_Store deleted successfully in directory ' + _distDir + '/' + obj );
    })
  })
  // gif folder
  ...
  fs.rm(path.join(_distDir, 'gif', '.DS_Store'), { recursive:true }, (err) => {
    ...
    console.log('DS_Store deleted successfully in directory ' + _distDir + '/gif' );
  })
}
...
function startAllZip(options) {
  ...
  fs.readFile(_distDir + '/index.html', 'utf-8', (err, data) => {
    ...
    indexContext = indexContext.replace(/(?<=href=\")(.*)(?=\">DOWNLOAD)/, allZipName + '.zip')

    fs.writeFile(_distDir + '/index.html', indexContext, 'utf-8', (err, data) => {
      if (err) throw err
